...
window.addEventListener("load", function() {
    // retrieve the query(field/value pairs) string from the URL
    var formData = location.search.slice(1);
    
    // remove all the + characters from the string and replace them with blank space characters
    formData = formData.replace(/\+/g," ");

    // decode all the URI character codes left in the string
    formData = decodeURIComponent(formData);

    // split the formData string into an array variable splitting the string by either the "=" or "&" characters
    var formFields = formData.split(/[&=]/g);

    // write the field values from the formFields array to the payment form
